...
#define PORT_UDP_H "43222"
#define PORT_UDP_M "44222"
#define LOCALHOST "127.0.0.1"
#define INPUT_H "history.txt"
#define MAXLINE 1024
...
socklen_t addrlen_M;
char buffer[MAXLINE];
map<std::string, int> book_status_H;
...
void create_socket_UDP() {
    ...
    if (getaddrinfo(LOCALHOST, PORT_UDP_H, &hints_udp, &res_udp) != 0) {     // first, load up address structs with getaddrinfo():
        perror("[ERROR] Server H: getaddrinfo failed");
        exit(1);
    }
    if ((sockfd_udp = socket(res_udp->ai_family, res_udp->ai_socktype, res_udp->ai_protocol)) == -1) {     // make a socket:
        perror("[ERROR] Server H: socket creation failed");
        exit(1);
    }
    
    if (bind(sockfd_udp, res_udp->ai_addr, res_udp->ai_addrlen) == -1) {   // bind it to the port we passed in to getaddrinfo():
        perror("[ERROR] Server H: bind socket failed");
        exit(1);
    }
...
void read_file() {
    FILE *fptr = fopen(INPUT_H, "r");
    if (fptr == NULL) {
        printf("File %s not found", INPUT_H);
        exit(1);
    }
    ...
    while(fgets(myString, 100, fptr)) {
        ...
        book_status_H[code] = num;
        // cout  << "cpp: "<< code << " " << book_status_H[code] << endl;
    }
    fclose(fptr);
}
...
void check_book() {
    ...
    if ( username != "firns" ) {
        printf("Server H received %s code from the Main Server.\n", bookcode.c_str());
    } else {
        printf("Server H received an inventory status request for code %s.\n", bookcode.c_str());
    }
    
    /*** check bookcode availability ***/
    int count = -1;
    if ( book_status_H.find(bookcode) != book_status_H.end() ) {
        count = book_status_H[bookcode];
        if ( book_status_H[bookcode] > 0 && username != "firns" ) { book_status_H[bookcode]--; }
    }
    ...
    if (sendto(sockfd_udp, buffer, strlen(buffer) + 1, 0, (struct sockaddr *) &addr_serverM, sizeof addr_serverM) < 0) {
        perror("[ERROR] Server H: fail to send book availability status to server M via UDP");
        exit(1);
    }
    if ( username != "firns" ) {
        printf("Server H finished sending the availability status of code %s to the Main Server using UDP on port %s.\n", bookcode.c_str(), PORT_UDP_H);
    } else {
        printf("Server H finished sending the inventory status to the Main server using UDP on port %s.\n", PORT_UDP_H);
    }
}

int main() {
    create_socket_UDP();
    printf("Server H is up and running using UDP on port %s.\n", PORT_UDP_H);
    read_file();
