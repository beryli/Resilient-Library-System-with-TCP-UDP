...
#define PORT_UDP_S "41222"
#define PORT_UDP_M "44222"
#define LOCALHOST "127.0.0.1"
#define INPUT_S "science.txt"
#define MAXLINE 1024
...
socklen_t addrlen_M;
char buffer[MAXLINE];
map<std::string, int> book_status_S;
...
void create_socket_UDP() {
    ...
    if (getaddrinfo(LOCALHOST, PORT_UDP_S, &hints_udp, &res_udp) != 0) {     // first, load up address structs with getaddrinfo():
        perror("[ERROR] Server S: getaddrinfo failed");
        exit(1);
    }
    if ((sockfd_udp = socket(res_udp->ai_family, res_udp->ai_socktype, res_udp->ai_protocol)) == -1) {     // make a socket:
        perror("[ERROR] Server S: socket creation failed");
        exit(1);
    }
    
    if (bind(sockfd_udp, res_udp->ai_addr, res_udp->ai_addrlen) == -1) {   // bind it to the port we passed in to getaddrinfo():
        perror("[ERROR] Server S: bind socket failed");
        exit(1);
    }
...
void read_file() {
    FILE *fptr = fopen(INPUT_S, "r");
    if (fptr == NULL) {
        printf("File %s not found", INPUT_S);
        exit(1);
    }
    ...
    while(fgets(myString, 100, fptr)) {
        ...
        book_status_S[code] = num;
        // cout  << "cpp: "<< code << " " << book_status_S[code] << endl;
    }
    fclose(fptr);
}
...
void check_book() {
    ...
    if ( username != "firns" ) {
        printf("Server S received %s code from the Main Server.\n", bookcode.c_str());
    } else {
        printf("Server S received an inventory status request for code %s.\n", bookcode.c_str());
    }
    
    /*** check bookcode availability ***/
    int count = -1;
    if ( book_status_S.find(bookcode) != book_status_S.end() ) {
        count = book_status_S[bookcode];
        if ( book_status_S[bookcode] > 0 && username != "firns" ) { book_status_S[bookcode]--; }
    }
    ...
    if (sendto(sockfd_udp, buffer, strlen(buffer) + 1, 0, (struct sockaddr *) &addr_serverM, sizeof addr_serverM) < 0) {
        perror("[ERROR] Server S: fail to send book availability status to server M via UDP");
        exit(1);
    }
    if ( username != "firns" ) {
        printf("Server S finished sending the availability status of code %s to the Main Server using UDP on port %s.\n", bookcode.c_str(), PORT_UDP_S);
    } else {
        printf("Server S finished sending the inventory status to the Main server using UDP on port %s.\n", PORT_UDP_S);
    }
}

int main() {
    create_socket_UDP();
    printf("Server S is up and running using UDP on port %s.\n", PORT_UDP_S);
    read_file();
